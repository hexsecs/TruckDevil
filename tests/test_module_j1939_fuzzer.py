...
def test_fuzzer_record_baseline(truckdevil_module_env, shared_channel):
    ...
    from truckdevil.libs.device import Device
    import truckdevil.modules.j1939_fuzzer as j1939_fuzzer
    ...
    try:
        ...
        t.start()
        time.sleep(0.05)
        with patch("truckdevil.modules.j1939_fuzzer.time.sleep"):
...
def test_fuzzer_save_load_smoke(truckdevil_module_env, shared_channel, tmp_path):
    ...
    try:
        ...
        try:
            ...
            j1939_fuzzer.main_mod(
                ["load", "fuzz_settings", str(f), "settings", "back"], device
            )
        finally:
            sys.stdout = old
    finally:
