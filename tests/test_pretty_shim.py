...
class PrettyShimImportTest(unittest.TestCase):
    ...
    def test_import_without_pretty_j1939(self):
        ...
        blocked = {
            "bitstring",
            "pretty_j1939",
            "pretty_j1939.describe",
            "pretty_j1939.render",
            "pretty_j1939.__main__",
        }
        original_import = (
        ...
        try:
            ...
            self.fail(
                "pretty_shim should not raise ImportError when pretty_j1939 is not installed"
            )
        finally:
    ...
    def test_no_unused_json_import(self):
        ...
        top_level_json_imports = [
            line for line in lines if line.strip().startswith("import json")
        ]
        self.assertEqual(
            len(top_level_json_imports),
            0,
...
class J1939ImportsTest(unittest.TestCase):
    ...
    def test_no_shlex_import_in_j1939(self):
        ...
        with open(_J1939_PY, "r") as f:
            source = f.read()
        lines = source.split("\n")
        shlex_imports = [line for line in lines if line.strip() == "import shlex"]
        self.assertEqual(
            len(shlex_imports), 0, "shlex should not be imported in j1939.py"
        )
